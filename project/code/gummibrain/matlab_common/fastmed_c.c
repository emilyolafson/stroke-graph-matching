...
static real_T quickSelect(const emxArray_real_T *arr, int32_T low, int32_T high, boolean_T exact);
...
void fastmed_c(const emxArray_real_T *x, const emxArray_real_T *mOutput)
{
    ...
    for(i = 0; i < dimensions; i++){
        mOutput[i] = quickSelect(x, i*samples,(i+1)*samples-1,exact);
    }
