             ...
             FibHeap  *theHeap  )
{
   ...
   while ((finished==0) && (ndone < M))
   {
      ...
      if (closestD == INF || closestD > maxdist) finished=1; else
      {
         ...
         for (i=startind; i<=endind; i++)
         {
            ...
            //mexPrintf( "INSPECT NEIGHBOR i=%d sr[i]=%f closestD=%f | #%d  olddist=%f newdist=%f\n" ,i, arclength, closestD, whichneighbor , olddist , closestD+arclength );            

            if ( olddist > ( closestD + arclength ) ) // || ( ndone==1 && i==startind ) )
            {
		 ...
		 )
{
  ...
  if (! ( nrhs == 2 || nrhs == 3))
  {
      mexErrMsgTxt( "Only 2 or 3 input arguments allowed." );
  }
      else if (nlhs != 1) 
   {
   ...
   if (mxIsSparse( prhs[ 0 ] ) == 1)
   {
     ...
     sr      = mxGetPr(prhs[0]);
     irs     = (size_t *) mxGetIr(prhs[0]);
     jcs     = (size_t *) mxGetJc(prhs[0]);

     //for (i=0; i<M; i++)
     //    mexPrintf("irs[%d]=%d, jrs[%d]=%d\n",i,irs[i],i,jcs[i]);
     ...
     for (i=0; i<MS; i++)
     {
        ...
        //mexPrintf("Heap nodes: %d\n",theHeap->GetNumNodes());
        
        S = (long int) *( SS + i );
        S--;

        //mexPrintf( "i=%d, S=%d, M=%d\n" , i, S, M);
        ...
        dodijk_sparse( M,N,S,maxdist,Dsmall,sr,irs,jcs,A,theHeap );
