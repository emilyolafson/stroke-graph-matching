function B = minfilt3(A,siz,padopt,CHUNKFACTOR)

%MEDFILT3 1-D, 2-D and 3-D minimum filtering.
...
existNaNmedian = exist('nanmin','file');
...
for i = 1:length(nchunk)-1
    ...
    if existNaNmedian
        B(nchunk(i):nchunk(i+1)) = nanmin(A(I0),[],2);
    else
        B(nchunk(i):nchunk(i+1)) = min(A(I0),[],2);
    end
end
B = cast(B,class0);
