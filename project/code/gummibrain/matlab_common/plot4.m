...
if(numel(varargin) >= 2 && strcmpi(varargin{1},'clim'))
    clim = sort(varargin{2});
    varargin = varargin(3:end);
else
    cvalid = c(~isnan(c));
    clim = [min(cvalid) max(cvalid)];
end
...
yy=linspace(clim(1),clim(2),size(cmap,1)); % Generate range of color indices that map to cmap
