...
hhistfill = fill(xi([1 end end 1]),[0 0 max(hx) max(hx)],'k','facecolor',[.75 .75 .75],'linestyle','none','facealpha',0,'tag','axhistfill');
hold on;
hbar = bar(xi,hx,1,'k','facecolor','k','tag','axhistbar');
...
hhistfill=findobj(axhist,'tag','axhistfill');
hbar=findobj(axhist,'tag','axhistbar');

%hhistfill = fill(axhist,xi([1 end end 1]),[0 0 max(hx) max(hx)],'k','facecolor',[.75 .75 .75],'linestyle','none','facealpha',0);
%hold(axhist,'on');
%hbar = bar(axhist,xi,hx,1,'k','facecolor','k');
set(hbar,'xdata',xi,'ydata',hx);
