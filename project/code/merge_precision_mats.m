...
clear dx_id
curr_dir=pwd;
subdir=strcat(curr_dir, '/stroke-graph-matching/data/precision')
...
for i=1:23
    for j=1:nsess(i)
        mat=C_precision{i,j}.C;
        mat(logical(eye(268)))=0;
       C_precision2{i,j}=mat;
       mats{i}=mat;
    end
end
...
%% %% save FC to matrices, rows as subjects, columns as sessions
curr_dir=pwd;
subdir=strcat(curr_dir, '/data/pearson_fc/')
fid = fopen(strcat(subdir, 'subjects.txt'));
line1 = fgetl(fid)
...
%    for j=1:nsess(i)
%        mat=C_precision{i,j}.C;
%       % mat=mat.*~logical(eye(268));
%        C_precision2{i,j}=mat;
%        mats{i}=mat;
%    end
%end

C_pearson=C_precision;
save(strcat(subdir, '/C_pearson.mat'), 'C_pearson')
