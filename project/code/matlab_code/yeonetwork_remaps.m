function [sums] = yeonetwork_remaps(remapping_freqs, curr_dir)
    ...
    % (separated by whether the node remaps from left to right, left to
    % left, right to right, and right to left).
    % input: permutation matrix using shen268 atlas
    ...
    remapping_freqs(logical(eye(268)))=0; %exclude diagonal elements (non-remaps)
    
    organized_matrix = remapping_freqs(yeo_labels,yeo_labels); %sort nodes by yeo assignment, which lists right then left
    ...
    for i=1:8
        for j=1:8
            ...
            sum_networks_ll_offdiagonal{i,j} = sum(sum(subnetwork));
        end
    end
    for i=1:8
        for j=1:8
            subnetwork = organized_matrixrr(networksr==i, networksr==j);
            sum_networks_rr_offdiagonal{i,j} = sum(sum(subnetwork));
        end
    end   
    for i=1:8
        for j=1:8
            subnetwork = organized_matrixrl(networksr==i, networksl==j);
            sum_networks_rl_offdiagonal{i,j} = sum(sum(subnetwork));
        end
    end  
    for i=1:8
        for j=1:8
            subnetwork = organized_matrixlr(networksl==i, networksr==j);
            sum_networks_lr_offdiagonal{i,j} =sum(sum(subnetwork));
