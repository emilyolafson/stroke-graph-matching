%% Find R peaks from ECG and add R markers to EEG
%                    
% amri_eeg_rpeak() 
%
% Usage
%   [OUTEEG]=amri_eeg_rpeak(EEG,ECG)
%
% Outputs
%   OUTEEG =  EEG data with R markers added
%
% Inputs
%   EEG   =   EEG data stored as an EEGLAB structure
%   ECG   =   ECG data stored as an EEGLAB strcuture
%             If ECG=[], find an ECG channel in EEG
%             or assume no ECG signal is available. 
% Keywords
%
% See also:
%   amri_sig_filtfft, 
...
%%
function outeeg = amri_eeg_rpeak(eeg,ecg,varargin)

if nargin<2
    error('amri_eeg_rpeak(): need at least two inputs');
end
...
checking       = 1;             % check intermediate results
...
for i = 1:2:size(varargin,2) % for each Keyword
    ...
    if strcmpi(Keyword,'checking')
        checking = Value;
    end
    
    if strcmpi(Keyword,'use_rmarker')
        use_rmarker = Value;
    end
    
    if strcmpi(Keyword,'rmarkername')
        r_marker.name = Value;
    end    
    
    if strcmpi(Keyword,'ecg_channel')
        ecg_channel = Value;
    end
    
    if strcmpi(Keyword,'kteo') && ~isempty(Value)
        kTEO = mergestruct(kTEO,Value);
    end
    
    if strcmpi(Keyword,'filter_method')
        filter_method = Value;
    end
end

if(ischar(ecg_channel))
    ecg_channel = StringIndex({ecg.chanlocs.labels},ecg_channel);
end

if(use_rmarker)
    [~,ev_r] = pop_selectevent(ecg,'type',r_marker.name);
    r_marker_latency = [ecg.event(ev_r).latency];
else
    r_marker_latency = [];
end
ecg = pop_selectevent(ecg,'omittype',r_marker.name,'deleteevents','on');
ecg = pop_select(ecg,'channel',ecg_channel);
...
if flag_verbose>0
    printf(['amri_eeg_rpeak(): construct k-TEO complex (k=' ...
        int2str(kTEO.k) ')']);
end
...
acorr = amri_sig_xcorr(Y,Y,'maxlag',60/min_pulse_rate*ecg.srate);   % autocorrelation
...
acorrrangepr=acorr(timerange>60/max_pulse_rate&timerange<60/min_pulse_rate);
...
for i=2:fix(length(Y)/(5*iri))
%for i=fix(.25*length(Y)/(5*iri)):fix(length(Y)/(5*iri))
    timerange = iri*5*(i-1)+iri+1: min([iri*5*i+iri length(Y)]);
    

    
    % an outlier differs from median_r_height by 3*std_r_height
    outliers = Y(timerange)>median_r_height+3*std_r_height;


    yd = Y(timerange) < max(median_r_height*.1, median_r_height-3*std_r_height);
    yd = [0 yd 0];
    d = diff(yd);
    d = [yd(1) d];
    d2 = d(d~=0); %
    d3 = diff(find(d ~= 0));
    if(all(size(d2(2:end)) == size(d3)))

        slen = d2(2:end).*diff(find(d ~= 0));
        longest_blank = max(-slen(slen < 0));
        if(longest_blank > 2*iri)
            outliers(1) = true;
        end
    else
        outliers(1) = true;
    ...
    %outliers = find(abs(Y(timerange)-median_r_height) > 3*std_r_height);
...
%%% if requested, use pre-existing rmarkers to identify a better template
%%% (takes mean of heartbeats and finds 1 heartbeat with max corr to mean)
if(use_rmarker && ~isempty(r_marker_latency))
    orig_template = template;
    
    %%
    r_windows = event_related_windows(Y,r_marker_latency,numel(template),'center');
    %%% throw out bad r_windows
    r_window_std = std(r_windows,[],1);
    bad_r_window = r_window_std > median(r_window_std) + std(r_window_std);
    r_windows = r_windows(:,~bad_r_window);
    
    r_mean = mean(r_windows,2);
    cc = corrfilter(r_windows,r_mean);
%     
%     datawin = event_related_windows(ecg.data,r_marker_latency,numel(template),'center');
%     new_latency = r_marker_latency;
%     for i = 1:1
%         if(i == 1)
%             template_type = 1;
%         else
%             template_type = 'mean';
%         end
%         new_latency = realign_data_windows(Y,new_latency,numel(template),template_type,'center');
% 
%         new_r_windows = event_related_windows(Y,new_latency,numel(template),'center');
%         new_r_window_std = std(new_r_windows,[],1);
%         new_bad_r_window = new_r_window_std > median(new_r_window_std) + std(new_r_window_std);
%         new_r_windows = new_r_windows(:,~new_bad_r_window);
%     end
%     
%     new_datawin = event_related_windows(ecg.data,new_latency,numel(template),'center');
%     
%     figure;
%     subplot(2,1,1);
%     plotalpha(datawin);
%     hold on;
%     plot(mean(datawin,2),'k','linewidth',3);
%     grid on;
%     
%     subplot(2,1,2);
%     plotalpha(new_datawin);
%     hold on;
%     plot(mean(new_datawin,2),'k','linewidth',3);
%     grid on;
%     
%     title('realigned');
%     UniformAxes;
...
%     plotalpha(r_windows);
%     hold on;
%     plot(r_mean,'k','linewidth',3);
%     grid on;
%     
%     subplot(2,1,2);
%     plotalpha(new_r_windows);
%     hold on;
%     plot(mean(new_r_windows,2),'k','linewidth',3);
%     grid on;
%     
%     title('realigned');
%     UniformAxes;
%     
    ...
    figure;
    imagesc(cc);
    [~,best_r_window] = max(max(cc,[],1));
    template = r_windows(:,best_r_window);
    %template = r_mean;
    
    
    if(checking >= 1)
        figure;
        plot(orig_template./max(orig_template));
        hold on;
        plot(r_mean./max(r_mean),'k');
        plot(template./max(template),'r');
        legend({'original','mean','best'});
    end
else
    if(checking >= 1)
        figure;
        plot(template./max(template));
    end
end
...
if(isnan(iri_std))
    error('did not find enough rpeaks.');
end
...
for j=1:length(fnames)
    if(ischar(eeg.event(1).(fnames{j})))
        newevent.(fnames{j})='';
    else
        newevent.(fnames{j})=[];
    end
...
for i=1:length(r_index)
    %for j=1:length(fnames)
    %    eval(['newevent.' fnames{j} '=[];']);
    %end
    newevent.latency = round(r_index(i)/ecg.srate*outeeg.srate);
    newevent.type = r_marker.name;
    newevent.duration = 1;
    %newevent.urevent = [];
    outeeg.event = [outeeg.event newevent];
end
