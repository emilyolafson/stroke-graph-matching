function [cutpos,facedata]=qmeshcut(esize,node,plane,edges)
%
...
emap(cutedges)=1:numel(cutedges);
if(esize(2)==10)
    emap=reshape(emap,[esize(1),12]); % 10-node element
else
	emap=reshape(emap,[esize(1),esize(2)*(esize(2)-1)/2]); % C^n_2
end
%faceid=any(emap,2)==1;
%facelen=sum(faceid);
...
if(esize(2)==3)  % surface mesh cut by a plane
	linecut=etag==2;
	lineseg=emap(linecut,:)';
	facedata=reshape(lineseg(lineseg>0),[2,sum(linecut)])';
	return;
end
...
tripatch=reshape(tripatch(tripatch>0),[3,sum(tricut)])';

% fast wall to get all quadrilaterals in convexhull form (avoid using convhulln)

quadpatch=emap(quadcut,:)';
quadpatch=reshape(quadpatch(quadpatch>0),[4,sum(quadpatch)])';
